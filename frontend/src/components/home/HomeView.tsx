...
export default function HomeView() {
  ...
  const [latestResultCentiDeck, setLatestResultCentiDeck] =
    useState<CentiDeckArtifactPayload | null>(null);
  ...
  useEffect(() => {
    ...
    const mode = latestResultSession.output_mode ?? "slidev";
    const run = async () => {
      if (mode === "html") {
        const artifact = await getLatestSessionPresentationCentiDeckArtifact(
          latestResultSession.id
        );
        if (cancelled) return;
        setLatestResultCentiDeck(artifact);
        setPreviewSlideIndex(0);
        return;
      }
      const slidev = await getLatestSessionPresentationSlidev(latestResultSession.id);
      if (cancelled) return;
                ...
                <div className="min-h-0 flex-1">
                  {(latestResultSession.output_mode ?? "slidev") === "html" ? (
                    ...
                    >
                      <p className="text-xs font-medium text-slate-500">
                        {Array.isArray(latestResultSlidev?.meta?.slides)
                          ? `Slidev 演示稿 · ${(latestResultSlidev.meta.slides as unknown[]).length} 页`
                          : "最近成果 · 预览尚未就绪"}
                      </p>
                      {Array.isArray(latestResultSlidev?.meta?.slides) && (
