    ...
    def _build_agent_presentation_prompt(self, job: GenerationJob, state: GenerationRuntimeState) -> str:
        ...
        if job.output_mode.value == "html":
            return (
                ...
                "请基于工作区素材和已确认的大纲，直接产出最终可播放、可继续编辑的 Reveal HTML deck。\n"
                "不要先设计结构化 slide schema、不要假设有模版系统、不要输出任何中间表示。\n\n"
                "工作方式：\n"
                ...
                "- 每页必须按 1280x720 的 16:9 画布设计，并确保内容完整落在画布内。\n"
                "- 必须是演示页面，不要写成长文档、滚动页面或海报长页。\n"
                "- 每个 `<section>` 都必须有稳定的 `data-slide-id=\"slide-N\"` 和 `data-slide-title=\"页面标题\"`。\n"
                "- 每页应自带完整的页面容器、版式层级和视觉设计，不要依赖外部模版类名才能成立。\n"
                "- 必须体现强视觉设计：鲜明但克制的主题、明确的版式层级、面向展示的文案密度、符合主题的艺术风格。\n"
                "- 优先做带艺术方向的 HTML PPT，不要退化成普通 markdown 页面。\n"
                "- 主要内容在首屏必须静态可见，禁止把核心文字放在初始 `opacity: 0`、`visibility: hidden`、`display: none` 或 fragment 状态。\n"
                "- 禁止依赖入场动画、hover、点击、JS 延迟执行后才让主要内容变得可读。\n"
                "- 禁止内容溢出画布、禁止把正文塞成过密的小字、禁止用空白占位块假装完成设计。\n"
