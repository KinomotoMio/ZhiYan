...
MessageTransform = Callable[[list[AgenticMessage]], list[AgenticMessage]]
...
def build_model(model_name=None) -> AgenticModelClient:
    """Build the default model client.

    The default implementation uses PydanticAI, but only through a thin adapter.
    """
...
async def agentic_loop(
    ...
    *,
    ...
    tool_definitions: Sequence | None = None,
    ...
    for turn in range(turn_limit):
        ...
        if dispatch_result.parts:
            messages.append(UserMessage(parts=list(dispatch_result.parts), instructions=_instructions(instructions)))
            if message_transform is not None:
                messages = message_transform(messages)
