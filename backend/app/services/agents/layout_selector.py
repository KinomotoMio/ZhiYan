...
输入：大纲 items + 可用布局清单
输出：每页的 group + variant + layout_id
...
class SlideLayoutChoice(BaseModel):
    ...
    variant: str = Field(
        default="default",
        description="先选择的组内变体；未试点 group 统一填 default",
    )
    ...
    if _agent is None:
        ...
        _agent = Agent(
            ...
            instructions=(
                ...
                "- 然后选择该 `group` 下的 `variant`，最后再落到具体 `layout_id`\n"
                "- 对未试点的 group，`variant` 一律填 `default`\n"
                ...
                "- `evidence` / `comparison` / `process` 仍按各自 group 内布局选择，但 variant 保持 `default`\n"
                ...
                "## 输出要求\n"
                "- 每页必须同时输出 `group`、`variant`、`layout_id`、`reason`\n"
                "- 若 `layout_id` 与你选定的 `group` / `variant` 不匹配，优先保证 `group` / `variant` 正确\n"
            ),
        )
    return _agent
