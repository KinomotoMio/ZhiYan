...
def test_summarize_state_includes_outline_layout_and_slide_progress():
    state = PipelineState(
        ...
        outline={
            "items": [
                {"slide_number": 1, "title": "问题定义"},
                {"slide_number": 2, "title": "市场趋势"},
                {"slide_number": 3, "title": "执行路径"},
            ]
        },
        layout_selections=[
    ...
    assert "大纲已生成：3 页 - 问题定义；市场趋势；执行路径" in summary
...
def test_attach_state_summary_wraps_tool_result_content():
    ...
    assert enriched.metadata["state_summary"] == "大纲已生成：1 页 - 问题定义"
    assert enriched.content == {
        "result": {"ok": True},
        "state_summary": "大纲已生成：1 页 - 问题定义",
    }
