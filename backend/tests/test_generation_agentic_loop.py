    ...
    async def _case():
        ...
        async def _dispatch(parts):
            ...
            return ToolDispatchResult(
                parts=[ToolResult(tool_name="parse_document", content={"status": "ok"}, tool_call_id=parts[0].tool_call_id)]
            )
...
def test_agentic_loop_stops_after_max_turns():
    async def _case():
        async def _dispatch(parts):
            return ToolDispatchResult(
                parts=[ToolResult(tool_name=parts[0].tool_name, content="ok", tool_call_id=parts[0].tool_call_id)]
            )
